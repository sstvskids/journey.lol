--[[
    ...
    by: Atonium Developers
    modified by: @stav
]]

getgenv().namehub = "journey.lol"
...
function Library:save_flags()
    ...
    writefile(`journey.lol/{game.GameId}.lua`, flags)
end

function Library:load_flags()
    if not isfile(`journey.lol/{game.GameId}.lua`) then Library.save_flags() return end

    local flags = readfile(`journey.lol/{game.GameId}.lua`)
...
function Library:new()
	...
	TextLabel.Position = UDim2.new(0.09, 0, 0.46, 9)
    ...
    function Tab:create_tab()
		...
		TextLabel.BorderColor3 = Color3.fromRGB(0, 0, 0)
		TextLabel.BorderSizePixel = 0
		TextLabel.Position = UDim2.new(0.58965224, 0, 0.4999999999, 0)
